...
-- Dialect: T-SQL (SQL Server) | Run this BEFORE inventory_query.sql
-- Soft delete: is_deleted = 1 (row hidden, not removed) | Archive: status = 'archived' (moved out of active view)


-- Drops the Inventory table if it exists to allow a clean re-run
IF OBJECT_ID('dbo.Inventory', 'U') IS NOT NULL DROP TABLE dbo.Inventory;
GO


-- Inventory: tracks ingredient stock levels with soft-delete and archive lifecycle support
...
-- Seeds 50 ingredients grouped by category; all rows default to active and non-deleted
...
-- Verifies the seed completed successfully; expected result: 50
