-- purchases_setup.sql  |  Cafe Saburo POS — Purchases Table Setup  |  T-SQL (SQL Server)
-- Line-item model: one row = one supplier + one inventory item; group by purchase_group_id for multi-item orders.
-- Run AFTER inventory_setup.sql and suppliers_setup.sql. Run this file once.

SET QUOTED_IDENTIFIER ON;
GO

-- ── TRUNCATE: Clear all rows safely without dropping FK constraints (for re-runs) ───────────────
...
-- ── DROP + RECREATE: Full reset — drops and rebuilds the table from scratch ──────────────────────
...
CREATE TABLE dbo.Purchases (
    ...
    -- Status flow: Pending → Approved → On its way → Delivered | Cancelled; archived = soft-hidden
...
-- ── SEED: 40 purchase line items across 15 order groups ──────────────────────────────────────────
...
-- ── VERIFY: Confirm row count and status distribution match expectations ─────────────────────────
-- Expected total: 40
